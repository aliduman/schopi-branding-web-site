...
export default function Footer({ dict, lang }: FooterProps) {
    const navigation = [
        { name: dict.footer.menu.product, href: `/${lang}/` },
        { name: dict.footer.menu.features, href: `/${lang}#features` },
        { name: dict.footer.menu.pricing, href: `/${lang}#pricing` },
        { name: dict.footer.menu.faq, href: `/${lang}#faq` },
    ];
    const legal = [
        { name: dict.footer.menu.privacyPolicy, href: `/${lang}/privacy` },
        { name: dict.footer.menu.termsOfService, href: `/${lang}/terms` },
        { name: dict.footer.menu.cookiePolicy, href: `/${lang}/cookie-policy` },
    ];
    const socials = [
        {
            label: "Twitter / X",
        ...
        {
            label: "LinkedIn",
            href: "https://linkedin.com/company/schopi",
            icon: (
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
    ...
    return (
        <footer className="bg-gray-50 dark:bg-[#080808] border-t border-gray-200 dark:border-white/8">
            <div className="container mx-auto px-6 pt-14 pb-8 max-w-7xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-10 mb-12">
                    ...
                    <div className="sm:col-span-2 lg:col-span-5 flex flex-col gap-5">
                        <Link href={`/${lang}`} className="inline-flex w-fit">
                            <SchopiLogo />
                        </Link>
                        <p className="text-slate-500 dark:text-white/40 text-sm leading-relaxed max-w-sm">
                            {dict.footer.description}
                        </p>

                        {/* Social icons */}
                        <div className="flex items-center gap-2">
                            <span className="text-slate-400 dark:text-white/30 text-xs mr-1">
                            ...
                            {socials.map((s) => (
                                <a
                                    ...
                                    aria-label={s.label}
                                    className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/15 text-slate-500 dark:text-white hover:bg-[#F84B18]/10 hover:text-[#F84B18] dark:hover:bg-[#F84B18]/20 dark:hover:text-[#F84B18] flex items-center justify-center transition-all duration-200 cursor-pointer"
                                >
                                    {s.icon}
                                </a>
                            ))}
                    ...
                    <div className="lg:col-span-3">
                        <p className="text-xs font-semibold tracking-widest uppercase text-[#F84B18] mb-4">
                            {dict.footer.menu.product}
                        </p>
                        <ul className="space-y-2.5">
                            {navigation.map((item) => (
                                <li key={item.name}>
                                    <Link
                                        href={item.href}
                                        className="text-sm text-slate-500 dark:text-white/45 hover:text-[#F84B18] dark:hover:text-[#F84B18] transition-colors duration-150 cursor-pointer"
                                    >
                                        {item.name}
                                    </Link>
                                </li>
                    ...
                    {/* Legal column */}
                    <div className="lg:col-span-3">
                        <p className="text-xs font-semibold tracking-widest uppercase text-slate-400 dark:text-white/25 mb-4">
                            Legal
                        </p>
                        <ul className="space-y-2.5">
                            {legal.map((item) => (
                ...
                {/* ── Bottom bar ── */}
                <div className="pt-6 border-t border-gray-200 dark:border-white/6 flex flex-col sm:flex-row items-center justify-between gap-3">
                    <p className="text-xs text-slate-400 dark:text-white/25">
                        © {new Date().getFullYear()} Schopi. All rights reserved.
                    </p>
